...
{
    /*write this*/
	assert(val != 0); //ensure val exists

	if (cur == 0) { //if current node is empty
		struct Node *newNode = (struct Node*)malloc(sizeof(struct Node)); //allocate memory for new node
		assert(newNode != 0); //ensure node was created
		newNode->left = 0; //set left and right values to null
		newNode->right = 0;
		newNode->val = val; //set new node value
		return newNode; //return new node
	}

	if (compare(cur->val, val) > 0) 
	{ //if passed value is less than current node value, add new node to left
		cur->left = _addNode(cur->left, val); 
	}
	else 
	{
		cur->right = _addNode(cur->right, val); //if passed value is more than current node value, add new node to right
	}
...
{
    /*write this*/
	assert(tree != 0); //ensure tree and val exist
	assert(val != 0);

	struct Node *searchNode = tree->root; //set first node to root

	while (searchNode != 0) //while search node isn't leaf
	{
		if (searchNode->val == val) //if val is equal, found
		{
			return 1; //Return true if found
		}
		else if (val < searchNode->val) //if val is less, move left
		{
			searchNode = searchNode->left; //set search node to left node
		}
		else 
		{
			searchNode = searchNode->right; //set search node to right node
		}
	}
...
{
    /*write this*/
	assert(cur != 0); //ensure cur exists

	while (cur->left != 0) //while not at leaf node
	{
		cur = cur->left; //set current node to left node
	}
	return cur->val; //return current value (left most)
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if (cur->left == 0) //if no left node, current node is left most, remove
	{ 
		struct Node *temp = cur->right; //set temp to node right of current
		free(cur); //remove current, free memory
		return temp; //return right node
	}
	
	else //if leaf node hasn't been reached, keep moving to the left
	{
		cur->left = _removeLeftMost(cur->left); //move to next left
		return cur; //return current node
	}
}
/*
...
{
    /*write this*/
	assert(cur != 0); //ensure cur and val exist
	assert(val != 0);
	
	if (compare(cur->val, val) == 0) //if current node value is what needs to be removed
	{
		if (cur->right == 0)  //if no node to right of current
		{ 
			struct Node *tempNode = cur->left; //set temp to current left node
			free(cur); //free current
			return tempNode; //return left node to take place of removed node
		}
	
	}
	else if (compare(cur->val, val) > 0) //if current node value is greater than passed value, move left
	{
		cur->left = _removeNode(cur->left, val);
	}
	else //if current node value is smaller than passed value, move right
	{
